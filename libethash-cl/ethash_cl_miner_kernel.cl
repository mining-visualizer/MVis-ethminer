...
/*-----------------------------------------------------------------------------------
* ethash_calculate_dag_item_slow
*----------------------------------------------------------------------------------*/
__kernel void ethash_calculate_dag_item_slow(uint start, __global hash64_t const* g_light, __global hash64_t * g_dag, uint isolate)
{
	uint const gid = get_global_id(0);
	...
	for (uint w = 0; w != ETHASH_DATASET_PARENTS; ++w) {
		uint parent_index = fnv(node_index ^ w, dag_node.words[w % NODE_WORDS]) % LIGHT_SIZE;
...
__kernel void ethash_calculate_dag_item(uint start, __global hash64_t const* g_light, __global hash64_t * g_dag, uint isolate)
{
	...
	hash200_t state;
	copy(state.uint4s, g_light[node_index % LIGHT_SIZE].uint4s, 4);
	state.words[0] ^= node_index;
	SHA3_512(state.uint2s, isolate);

	// work items team up to help each other compute their dag item
	uint const team_member_id = gid & (TEAM_SIZE - 1);
	uint const team_id = get_local_id(0) / TEAM_SIZE;

	__local compute_hash_share share[GROUP_SIZE / TEAM_SIZE];
	__local uint parent_index[GROUP_SIZE / TEAM_SIZE];
	__local uint nodeIndexShared[GROUP_SIZE / TEAM_SIZE];

	for (uint i = 0; i < TEAM_SIZE; i++)
	{
		// one team member copies their mix into shared memory
		if (i == team_member_id)
		{
			copy(share[team_id].ulongs, state.ulongs, 8);
			nodeIndexShared[team_id] = node_index;
		}
		barrier(CLK_LOCAL_MEM_FENCE);

		// each team member copies a portion of that down into their private mix
		uint2 mix = share[team_id].uint2s[team_member_id];
		barrier(CLK_LOCAL_MEM_FENCE);

		for (uint w = 0; w != ETHASH_DATASET_PARENTS; ++w)
		{
			if ((w % NODE_WORDS / 2) == team_member_id)
				parent_index[team_id] = fnv(nodeIndexShared[team_id] ^ w, ((uint*) &mix)[w % 2]) % LIGHT_SIZE;
			barrier(CLK_LOCAL_MEM_FENCE);
			mix = fnv2(mix, g_light[parent_index[team_id]].uint2s[team_member_id]);
		}

		share[team_id].uint2s[team_member_id] = mix;
		barrier(CLK_LOCAL_MEM_FENCE);

		if (i == team_member_id)
			copy(state.ulongs, share[team_id].ulongs, 8);
		barrier(CLK_LOCAL_MEM_FENCE);
	}

	SHA3_512(state.uint2s, isolate);
	copy(g_dag[node_index].uint4s, state.uint4s, 4);
}
